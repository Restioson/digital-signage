...
class DatabaseController:
    ...
    def create_department(self, department: Department, insert_people=False) -> int:
        ...
        with self.db:
            cursor = self.db.cursor()
            cursor.execute(
                "INSERT INTO departments (name, bio, timetable) VALUES (?, ?, ?)",
                (department.name, department.bio, department.timetable),
            )
    ...
    def create_display_group(self, group: DisplayGroup, department_id: int) -> int:
        ...
        with self.db:
            ...
            cursor.execute(
                ...
                " VALUES (?, ?, ?)",
                (group.name, department_id, json.dumps(group.layout_xml)),
            )
